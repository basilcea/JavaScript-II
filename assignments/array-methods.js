...
let largeShirts = [];
runners.filter(runner => runner.shirt_size === 'L' ? largeShirts.push(runner) : null)
...
// Problem 1
// The event Manager needs to know the five highest donor companies and needs the donation to be sorted in descending order of value
let topFiveDonors =[]
let sortedDonation = runners.sort((a, b) => b.donation - a.donation)
sortedDonation.filter(runner => sortedDonation.indexOf(runner) < 5 ? topFiveDonors.push(runner.company_name): null )
console.log(topFiveDonors);
// Problem 2
// The event manager needs all name and email of representaives of company with shirt size of 3xL to apologize for inability to provide their shirt except for one person as they have only have one 3xl shirt.

let largeSizedReps = [];
runners.filter(runner => runner.shirt_size ==='3XL'? largeSizedReps.push(` first_name: ${runner.first_name} last_name: ${runner.last_name}  email: ${runner.email}`): null)
largeSizedReps.shift()
console.log(largeSizedReps)

// Problem 3
// get all companies starting from A to D
const companyList =['A', 'B' , 'C' ,'D']
let companies = [];
runners.filter(runner => companyList.includes(runner.company_name.toUpperCase().charAt(0))? companies.push(runner) : null  )
